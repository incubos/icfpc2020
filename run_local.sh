...
while read line; do
  if [[ ! -z $(echo "$line" | grep OTHER_PLAYER_KEY_SHOULD_BE) ]]; then
    ...
    tee -a games/full_2.log | grep -E '(Sending to server)|(response demList)|(Start demList)|(Join demList)' >> games/2.log; # Log only important info
  fi;
  echo "$line" >> games/full_1.log;
  # Long only important info
  if [[ ! -z "$(echo "$line" | grep -E '(Sending to server)|(response demList)|(Start demList)|(Join demList)')" ]]; then
