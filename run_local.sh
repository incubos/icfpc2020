...
while read line; do
  if [[ ! -z $(echo "$line" | grep OTHER_PLAYER_KEY_SHOULD_BE) ]]; then
    ...
    tee -a games/full_2.log | grep -E '(Sending join request)|(Join response)|(Sending start request)|(Start response)|(Sending to server)|(Command command response)' >> games/2.log; # Log only important info
  fi;
  echo "$line" >> games/full_1.log;
  # Long only important info
  if [[ ! -z "$(echo "$line" | grep -E '(Sending join request)|(Join response)|(Sending start request)|(Start response)|(Sending to server)|(Command command response)')" ]]; then
